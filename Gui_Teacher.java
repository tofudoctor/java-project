...
public class Gui_Teacher extends JFrame {
	...
	public Gui_Teacher() throws IOException{
		...
		JLabel Openline = new JLabel("歡迎"+Main.users.get(Main.loginemail).getName()+"老師");
        	...
        	public void mouseClicked(MouseEvent e)  {
				...
				if(operation_option == 0){
					...
					if(Main.books.get(name) == null){
						...
						if(Main.users.get(Main.loginemail).getCount()<5){
							...
							JOptionPane.showMessageDialog(null, "最多借5本!!!");
						}
					}
        	...
        	public void mouseClicked(MouseEvent e)  {
				...
				if(operation_option == 0){
					...
					if(Main.books.get(name).day_difference(day, Main.books.get(name).getReturn_day(), year, Main.books.get(name).getReturn_year()) < 0){// overdue
						...
						if(Main.books.get(name).getReturn_day()+15>365){
							Main.books.get(name).setReturn_day(Main.books.get(name).getReturn_day()+15);
