...
public class Gui_Student extends JFrame {
	...
	public Gui_Student() throws IOException{
		...
		JLabel Openline = new JLabel("歡迎"+Main.users.get(Main.loginemail).getName()+"同學");
        	...
        	public void mouseClicked(MouseEvent e)  {
				...
				if(operation_option == 0){
					...
					if(Main.books.get(name) == null){
						...
						if(Main.users.get(Main.loginemail).getCount()<2){
        	...
        	public void mouseClicked(MouseEvent e)  {
				...
				if(operation_option == 0){
					...
					if(Main.books.get(name).day_difference(day, Main.books.get(name).getReturn_day(), year, Main.books.get(name).getReturn_year()) < 0){// overdue
						...
						if(Main.books.get(name).getReturn_day()+5>365){
							Main.books.get(name).setReturn_day(Main.books.get(name).getReturn_day()+5);
